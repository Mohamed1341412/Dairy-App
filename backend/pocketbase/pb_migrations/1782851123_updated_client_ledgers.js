...
migrate((app) => {
  const collection = app.findCollectionByNameOrId("pbc_2899278562")

  // update collection data
  unmarshal({
    "indexes": [
      "CREATE INDEX `idx_client_ledgers_client` ON `client_ledgers` (`client_id`)",
      "CREATE UNIQUE INDEX `idx_client_ledgers_tx_entry` ON `client_ledgers` (\n  `transaction_id`,\n  `entry_type`\n)",
      "CREATE INDEX `idx_client_ledgers_replay_order` ON `client_ledgers` (`posting_sequence`)"
    ]
  }, collection)
