...
const PaymentAllocationService = {
    ...
    _validatePayment: function(payment, isReverse) {
        if (!payment || !payment.id) {
            throw new Error('Payment is required.');
        }
        
        const expectedStatus = isReverse ? 'confirmed' : 'confirmed';
        if (payment.get('status') !== expectedStatus) {
            throw new Error(`Payment must be ${expectedStatus}. Current status: ${payment.get('status')}`);
        }
    },
    ...
    _validateAllocations: function(allocations) {
        ...
        allocations.forEach(alloc => {
            ...
            if (!amount || amount <= 0 || isNaN(amount)) {
                throw new Error(`Invalid allocated_amount: ${amount}. Must be greater than 0.`);
            }
        });
    },
    ...
    _validateNoDuplicateTransactions: function(allocations) {
        ...
        if (txIds.length !== uniqueTxIds.length) {
            throw new Error('Cannot allocate to same transaction multiple times in one operation.');
        }
    },
    ...
    _validateTransactions: function(allocationsWithTx, isReverse) {
        allocationsWithTx.forEach(({ allocation, transaction }) => {
            ...
            if (!isReverse && transaction.get('affects_cashflow')) {
                throw new Error(`Cannot allocate to cashflow transaction: ${transaction.id}`);
            }
            
            // Transaction must not be reversed (only for execute, not reverse)
            if (!isReverse && transaction.get('status') === 'reversed') {
                throw new Error(`Cannot allocate to reversed transaction: ${transaction.id}`);
            }
            ...
            if (!isReverse) {
                ...
                if (allocated > remaining) {
                    throw new Error(`Allocation ${allocated} exceeds remaining ${remaining} for transaction ${transaction.id}`);
                }
            }
        });
    },
    ...
    _validatePartyConsistency: function(payment, allocationsWithTx) {
        ...
        allocationsWithTx.forEach(({ transaction }) => {
            ...
            if (paymentParty.type !== txParty.type || paymentParty.id !== txParty.id) {
                throw new Error(
    ...
    _validateTotals: function(payment, allocations, isReverse) {
        ...
        if (!isReverse) {
            const totalAllocated = allocations.reduce((sum, alloc) => 
                sum + alloc.getFloat('allocated_amount'), 0);
            
            if (totalAllocated > payment.getFloat('amount')) {
                throw new Error(`Total allocations ${totalAllocated} exceed payment amount ${payment.getFloat('amount')}.`);
            }
        }
    },
    ...
    _validate: function(dao, payment, allocations, isReverse = false) {
        // 1. Basic validations
        this._validatePayment(payment, isReverse);
        ...
        const allocationsWithTx = allocations.map(alloc => {
            const tx = dao.findRecordById('transactions', alloc.get('transaction_id'));
            if (!tx) {
                throw new Error(`Transaction not found: ${alloc.get('transaction_id')}`);
            }
    ...
    _applyAllocation: function(allocation, transaction) {
        ...
        const remainingAmount = transaction.getFloat('remaining_amount') - allocatedAmount;
    ...
    _recalculateUnallocatedAmount: function(dao, payment) {
        ...
        payment.set('unallocated_amount', payment.getFloat('amount') - totalAllocated);
