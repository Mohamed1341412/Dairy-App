...
const AuditService = {
  log({ userId = null, action, operationType, collectionName, recordId }) {
    if (!action || !operationType || !collectionName || !recordId) {
      console.error('[AuditService] Missing required fields', { action, operationType, collectionName, recordId });
      return;
    }
