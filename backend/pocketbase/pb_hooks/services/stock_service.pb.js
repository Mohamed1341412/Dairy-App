...
const { Collections, MovementDirections } = require(`${__hooks}/core/collections.pb.js`);
...
const StockService = {
  ...
  incrementalUpdate: (movement) => {
    ...
    try {
      ...
      console.error(`Failed incremental stock update for product ${productId}:`, err);
      throw err;
    }
  },
  ...
  validateNegativeStock: (productId, requestedQty) => {
    ...
    try {
      ...
      throw new BadRequestError(`Stock validation failed: Product with ID '${productId}' not found.`);
    }

    const available = product.getFloat('available_stock');
    if (available < requestedQty) {
      throw new BadRequestError(`Insufficient stock for product: '${product.get('name')}'. Available: ${available}, Requested: ${requestedQty}.`);
    }
  }
};
