...
onRecordBeforeUpdateRequest((e) => {
    ...
    if (oldStatus !== 'draft' && newStatus !== 'draft') {
        ...
        for (const field of protectedFields) {
            if (e.oldRecord.get(field) !== e.record.get(field)) {
                throw new Error(`Field '${field}' cannot be modified after order confirmation.`);
            }
        }
    }
    ...
    const businessDate = e.record.get('business_date') || e.record.get('order_date');
    
    // ==========================================
    // EVENT: Draft → Confirmed (Reserve Stock)
    // ==========================================
    if (oldStatus === 'draft' && newStatus === 'confirmed') {
        const items = $app.dao().findRecordsByFilter('sales_order_items', `sales_order_id = "${orderId}"`);
        
        // Reserve stock (reserveStock already calls ensureAvailableStock internally)
        for (const item of items) {
            StockService.reserveStock($app.dao(), 'products', item.get('product_id'), item.getFloat('quantity'));
        }
    ...
    // ==========================================
        ...
        for (const item of items) {
            ...
            const movementCollection = $app.dao().findCollectionByNameOrId('inventory_movements');
        ...
        LedgerService.projectTransaction($app.dao(), txRecord, 'receivable', reference);
    ...
    // ==========================================
        ...
        // Release reservation
        for (const item of items) {
            StockService.releaseReservation($app.dao(), 'products', item.get('product_id'), item.getFloat('quantity'));
        }
    ...
    // ==========================================
        ...
        const originalTx = $app.dao().findFirstRecordByFilter('transactions', `reference_collection = "sales_orders" && reference_id = "${orderId}"`);
        ...
        const reference = { type: 'sales_orders', id: orderId, number: orderRef };
        LedgerService.projectTransaction($app.dao(), reversalTx, 'reversal', reference);
...
onRecordBeforeUpdateRequest((e) => {
    ...
    if (e.collection.name === 'sales_order_items') {
        ...
        try {
            ...
            if (status !== 'draft') {
                throw new Error(`Cannot modify items of a ${status} order. Only draft orders can have items modified.`);
            }
        } catch (err) {
