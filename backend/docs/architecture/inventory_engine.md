...
- Domain Hooks call `StockService.applyMovement(dao, movement)` inside their transactions to update stock projections adds/subtracts quantity to/from `products.current_stock`.
- `available_stock` = `current_stock - reserved_stock` (where `reserved_stock` is managed by `StockService.reserveStock()` and `releaseReservation()` called by `sales_hooks`).
