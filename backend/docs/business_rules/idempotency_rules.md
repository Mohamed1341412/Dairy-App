...
|------------|-----|------------------------|
| **Sales Confirm** | `sales_order_id` + `event:confirm` | If `inventory_reservations` exists → abort |
| **Sales Deliver** | `sales_order_id` + `event:deliver` | If `inventory_movements` exists → abort |
