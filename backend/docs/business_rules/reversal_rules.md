...
- `reversal_of` (relation to the original transaction) – required for financial transactions
- `corrects_movement_id` (relation to the original inventory movement) – required for inventory corrections
...
- `corrects_movement_id` = `id` of the original movement

### Execution Flow:

Domain Hooks (not inventory_hooks) will:

1. Create the reversal inventory_movement record
2. Call StockService.applyMovement() inside the same transaction
3. StockService updates `products.current_stock` and `product_batches.quantity_remaining`

### Example:

Original: Sale = `10 units`, `direction = out`
Reversal: Inventory Movement = `10 units`, `direction = in`, `movement_type = adjustment`, `corrects_movement_id = [original_id]`
